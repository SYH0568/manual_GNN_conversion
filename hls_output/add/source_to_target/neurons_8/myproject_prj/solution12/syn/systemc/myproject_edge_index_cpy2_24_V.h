...
#ifndef _myproject_edge_index_cpy2_24_V_H_
#define _myproject_edge_index_cpy2_24_V_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "myproject_edge_index_cpy2_24_V_memcore.h"

SC_MODULE(myproject_edge_index_cpy2_24_V) {
...
protected:
    ...
    // buffer instances
    myproject_edge_index_cpy2_24_V_memcore* buffer[BufferCount];
...
public:
    ...
    SC_CTOR(myproject_edge_index_cpy2_24_V) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "myproject_edge_index_cpy2_24_V_memcore_U_";
            name += szTmp;

            buffer[i] = new myproject_edge_index_cpy2_24_V_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "myproject_edge_index_cpy2_24_V.trace");
            // ports
